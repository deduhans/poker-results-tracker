<template>
  <div>
    <v-btn icon variant="text" color="primary" @click="dialog = true" data-cy="share-room-button">
      ...
      <v-card>
        ...
        <v-card-text class="text-center pa-4">
          <div class="d-flex justify-center mb-4">
            <qrcode-vue :value="roomUrl" :size="200" level="H" render-as="svg" data-cy="room-qr-code" />
          </div>
          ...
          <v-btn block color="primary" @click="copyToClipboard" class="mt-2" data-cy="copy-room-url-button">
            <v-icon start>mdi-content-copy</v-icon>
            Copy Link
          </v-btn>

          <v-btn v-if="isHost" block color="secondary" @click="regenerateToken" class="mt-2" :loading="regenerating"
...
const isHost = computed(() => {
  const hostPlayer = props.room.players.find(player => player.role === PlayerRoleEnum.Host);
