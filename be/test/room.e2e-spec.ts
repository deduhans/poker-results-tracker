...
describe('RoomController (e2e)', () => {
  ...
  describe('GET /rooms', () => {
    it('should get all rooms', async () => {
      // First create a room
      await TestHelper.createTestRoom(app, accessToken, testUser.id);
  ...
  describe('GET /rooms/:id', () => {
    ...
    it('should require authentication', () => {
      return request(app.getHttpServer()).get(`/rooms/${testRoom.id}`).expect(401);
    });
  });
  ...
  describe('PUT /rooms/close/:id', () => {
    ...
    it('should close a room with balanced results', async () => {
      ...
      await request(app.getHttpServer()).post('/exchanges').set('Authorization', `Bearer ${accessToken}`).send({
        roomId: testRoom.id,
